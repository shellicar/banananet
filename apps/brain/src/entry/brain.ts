...
const main = async () => {
  ...
  logger.info(`Starting brain v${versionInfo.version} (${versionInfo.shortSha}) built ${versionInfo.buildDate} | docker: ${dockerBuildHash} built ${dockerBuildTime}`, { component: 'lifecycle' });

  const { signal, controller } = createShutdownController({ delayBeforeAbort: 240_000, gracePeriod: 60_000 });

  const { CONTAINER_APP_PORT, CLAUDE_CONFIG_DIR, CLAUDE_SDK_CWD, CLAUDE_SDK_DEFAULT_MAXTURNS, CLAUDE_SDK_WORKSPACE_MAXTURNS, BOT_ALIASES, WORKSPACE_COMMANDS, AUDIT_DIR, CALLBACK_HEADERS } = brainSchema.parse(env, { reportInput: true });
  ...
  createHono({ audit, sdkConfig, callbackHeaders: CALLBACK_HEADERS, abortController: controller, signal, port: CONTAINER_APP_PORT });
};
